            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="client" id="client" class="w-full p-2 border border-gray-300 rounded-lg shadow-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 transition duration-200" placeholder="Nombre del cliente" value="{{ old('client', $event->client) }}">
                        @error('client')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select name="type" id="type" class="w-full px-3 py-2 border border-gray-300 rounded-lg shadow-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 transition duration-200">
                                ...
                                <option value="Section" {{ old('type', $event->type) == 'Section' ? 'selected' : '' }}>Sección</option>
                                <option value="Instagram" {{ old('type', $event->type) == 'Instagram' ? 'selected' : '' }}>Instagram</option>
                            </select>
                            @error('type')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
    ...
    <script>
        ...
        async function deleteImage(button) {
            ...
            try {
                ...
                const response = await fetch(`{{ route('event.images.destroy', ':id') }}`.replace(':id', imageId), {
