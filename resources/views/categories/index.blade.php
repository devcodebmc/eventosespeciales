        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="flex flex-col sm:flex-row justify-between items-center py-6 px-4 sm:px-6 space-y-4 sm:space-y-0">
                    <form method="POST" action="{{ route('categories.store') }}" class="flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-4 w-full sm:w-auto">
                        @csrf
                        <!-- Nombre de la categoria -->
                        <div class="flex items-center space-x-2 w-full sm:w-auto">
                            <input id="name" type="text" name="name" value="{{ old('name') }}" placeholder="Nueva categoría" required class="block w-full border border-gray-300 rounded-md pl-3 text-sm" oninput="this.value = this.value.charAt(0).toUpperCase() + this.value.slice(1)"/>
                        </div>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach ($categories as $category)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        {{ (strlen($category->description) > 40) ? substr($category->description, 0, 20) . '...' : (string)$category->description }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @if ($category->image)
                                            <img src="{{ asset('storage/' . $category->image) }}" alt="Imagen actual" class="w-12 h-12 object-cover rounded-md border border-gray-200">
                                        @else
                                            Sin imagen
                                        @endif
                                    </td>
