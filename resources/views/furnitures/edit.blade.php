                    ...
                    <form method="POST" action="{{ route('furnitures.update', $furniture->id) }}" enctype="multipart/form-data">
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            ...
                            <div class="col-span-1">
                                ...
                                <input id="name" type="text" name="name" value="{{ old('name', $furniture->name) }}" required class="mt-1 block w-full border border-gray-300 rounded-md pl-3 text-sm" oninput="this.value = this.value.charAt(0).toUpperCase() + this.value.slice(1)"/>
                            </div>
                            ...
                            <div class="col-span-1">
                                ...
                                <textarea id="description" name="description" class="mt-1 block w-full border border-gray-300 rounded-md pl-3 text-sm resize-none h-32" rows="4" oninput="this.value = this.value.charAt(0).toUpperCase() + this.value.slice(1)">{{ old('description', $furniture->description) }}</textarea>
                            </div>
                            ...
                            <div class="flex items-center space-x-2">
                                <select id="service_id" name="service_id" class="mt-1 block w-full rounded-md border border-gray-300 bg-white py-2 px-3 text-sm shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 transition duration-150 ease-in-out">
                                    ...
                                    @foreach ($services as $service)
                                        <option value="{{ $service->id }}" {{ old('service_id', $furniture->service_id) == $service->id ? 'selected' : '' }}>{{ $service->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="col-span-1">
                                ...
                                >
                                    <option value="draft" {{ old('status', $furniture->status) == 'draft' ? 'selected' : '' }}>Borrador</option>
                                    <option value="published" {{ old('status', $furniture->status) == 'published' ? 'selected' : '' }}>Publicado</option>
                                </select>
                            </div>
                            ...
                            <div class="col-span-1">
                                ...
                                <input 
                                    ...
                                    id="order" 
                                    name="order" 
                                    value="{{ old('order', $furniture->order) }}"
                                    min="1"
                                    max="{{ $orderLimit }}"
                            ...
                            <div class="col-span-1">
                                ...
                                <div class="mt-2">
                                    <div class="flex justify-center">
                                        @if($furniture->image)
                                            <img id="image-preview" src="{{ asset('storage/' . $furniture->image) }}" alt="Imagen actual" class="w-24 h-24 object-cover rounded-md border border-gray-200"/>
                                        @else
                        ...
                        <div class="flex items-center mt-4">
                            <a href="{{ route('furnitures.index') }}" class="text-blue-500 hover:underline">
                                Volver a Mobiliario
                            </a>
                        </div>
                        @error('name')
