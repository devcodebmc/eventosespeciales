@if($quotations->count())
    <div id="historyGrid" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 2xl:grid-cols-4 gap-4">
        @include('quotations._quotation_cards', ['quotations' => $quotations])
    </div>

    <div id="loadMoreWrap" class="mt-6 sm:mt-8 flex flex-col items-center gap-3 {{ $hasMore ? '' : 'hidden' }}">
    ...
    <p id="historyEndMsg" class="mt-6 sm:mt-8 text-center text-xs text-[var(--cz-ink-soft)] {{ $hasMore ? 'hidden' : '' }}">
